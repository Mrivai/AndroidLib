...
namespace Mrivai.Pelitabangsa
{
    ...
    internal static class Ngrok
    {
        ...
        /// <para>Never try to start/kill the running Ngrok, as the <see cref="AndroidController"/> type handles it internally.</para></remarks>
        ...
        internal static string ExecuteNgrokCommand(NgrokCommand command)
        {
            ...
            lock (_lock)
            {
                result = Command.RunProcessReturnOutput(@AndroidController.Instance.ResourceDirectory + NGROK_EXE, command.Command, command.Timeout);
            }
            return result;
        }
        /// <summary>
        ...
        internal static void ExecuteNgrokCommandNoReturn(NgrokCommand command)
        {
            lock (_lock)
            {
                Command.RunProcessNoReturn(@AndroidController.Instance.ResourceDirectory + NGROK_EXE, command.Command, command.Timeout);
            }
        }
    }
}
