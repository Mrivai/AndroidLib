...
namespace Mrivai.Pelitabangsa
{
    ...
    public static class AdbCmd
    {
        ...
        internal const string ADK_EXE = "linux-adk.exe";
        ...
        /// <param name="command">Instance of <see cref="AdbCommand"/></param>
        /// <returns>Output of <paramref name="command"/> run on server</returns>
        public static void ExecuteAdbCommandExitWait(AdbCommand command)
        {
            lock (_lock)
            {
                 Command.RunProcessWaitReturnOutput(AndroidController.Instance.ResourceDirectory + ADB_EXE, command.Command);
            }
        }
        ...
        internal static bool GetSS(string folder)
        {
            bool sc = false;
            //Process.Start(AndroidController.Instance.ResourceDireC:\Users\mrivai89\Documents\Mrivai\Source Code\Xiaomi Service Tool\RN3 PRO TOOLKIT\Resources\ctory + "ss.exe");
            var Arguments = @AndroidController.Instance.ResourceDirectory + ADB_EXE + " exec-out screencap -p > " + folder;
        ...
        /// <summary>
        /// Executes an <see cref="AdbCommand"/>Linux Adk
        /// </summary>
        /// <remarks>This should be used if you want the output of the command returned</remarks>
        /// <param name="command">Instance of <see cref="AdbCommand"/></param>
        /// <param name="forceRegular">Forces Output of stdout, not stderror if any</param>
        /// <returns>Output of <paramref name="command"/> run on server</returns>
        public static string ExecuteLinuxAdkCommand(AdbCommand command, bool forceRegular = false)
        {
            string result = "";

            lock (_lock)
            {
                result = Command.RunProcessReturnOutput(AndroidController.Instance.ResourceDirectory + ADK_EXE, command.Command, forceRegular, command.Timeout);
            }
