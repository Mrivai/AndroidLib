    ...
    public class Flash
    {
        ...
        public void StartFlash()
        {
            try
            {
                if (device.State == DeviceState.FASTBOOT)
                {
                    ...
                    //FastbootFlasher();
                    new Thread(new ThreadStart(FastbootFlasher)).Start();
                }
                else if (device.State == DeviceState.EDL)
                {
                    Logger.w("start", "Edl flasher:", null);
                    //EDLFlasher();
                    new Thread(new ThreadStart(EDLFlasher)).Start();
                }
            }
            catch (Exception ex)
            {
        ...
        private void FastbootFlasher()
        {
            ...
            try
            {
                ...
                string command = string.Format("pushd \"{0}\"&&prompt $$&&set PATH=\"{1}\";%PATH%&&\"{2}\" -s {3}&&popd", SwPath, AndroidController.Instance.ResourceDirectory, str, name);
                Logger.w("Firmware path: " + SwPath, "Fastboot Flasher:", null);
                Logger.w("env android path: " + AndroidController.Instance.ResourceDirectory, "Fastboot Flasher:", null);
        ...
        private void EDLFlasher()
        {
            ...
            else if (Directory.Exists(SwPath + "images"))
            {
                Logger.w("sw path is not valid", "Edl flasher:", null);
                throw new Exception("sw path is not valid");
            }
        ...
        private string[] getDevicesQc()
        {
            ...
            string qcLsUsb = AndroidController.Instance.ResourceDirectory + "lsusb.exe";
